...
public class MerchantWalletService {
    ...
    private ShopSettlementView settlementView(ShopSettlement s) { OrderInfo o = orderMapper.selectById(s.getOrderId()); TradeOrder t = tradeMapper.selectById(s.getTradeId()); return new ShopSettlementView(id(s.getId()), id(s.getShopId()), id(s.getOrderId()), o == null ? null : o.getOrderNo(), id(s.getTradeId()), t == null ? null : t.getTradeNo(), s.getStatus(), money(s.getGrossAmount()), money(s.getBuyerPaidAmount()), money(s.getPlatformCouponSubsidyAmount()), money(s.getShopCouponDiscountAmount()), s.getCommissionRate() == null ? "0.0000" : s.getCommissionRate().setScale(4, RoundingMode.HALF_UP).toPlainString(), Boolean.TRUE.equals(s.getCommissionRefundable()), money(s.getCommissionAmount()), money(s.getBuyerRefundAmount()), money(s.getPlatformSubsidyRefundAmount()), money(s.getCommissionRefundAmount()), money(s.getMerchantRefundAmount()), money(s.getNetAmount()), money(s.getPendingAmount()), money(s.getReleasedAmount()), time(s.getAvailableAt()), time(s.getSettledAt()), time(s.getCreatedAt()), time(s.getUpdatedAt())); }
