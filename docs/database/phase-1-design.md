...
> 文档关系说明：文件名中的 `phase-1` 是历史名称，本文定义的 26 张表是[一期](../product/phase-1-requirements.md)和[二期](../product/phase-2-requirements.md)共同使用的完整数据库基线。产品范围、HTTP 契约和前后端协作规范统一从[文档索引](../README.md)进入；产品阶段不会另起一套相互冲突的数据模型。
...
- [`sql/schema2.sql`](../../sql/schema2.sql) 是基线之后的二期增量迁移，包含新增权限、授权和既有种子元数据调整，可重复执行。
