...
本文是时光商城数据库的最终版结构说明，与 [`sql/schema.sql`](../../sql/schema.sql) 配套使用。本文负责说明数据模型、字段语义、表关系和业务约束；`schema.sql` 负责定义可直接执行的 MySQL 物理结构。修改任一表、字段、索引、外键、状态码或金额公式时，必须同步修改两份文件。
...
| 主键 | 独立业务实体使用 `BIGINT UNSIGNED AUTO_INCREMENT`；纯关系表使用复合主键 |
...
种子角色包括：普通用户、平台店铺管理员、平台商品审核员、超级管理员、店铺管理员、店铺商品运营、店铺订单客服和店铺库存人员。种子权限及角色权限映射以 `schema.sql` 第 8 节为准。
...
一份 `after_sale_request` 只对应一条 `order_item`。同一明细可以存在多份售后申请，但应用必须按其他已批准申请占用的数量和金额计算本次可批准额度。
...
- 售后申请数量和金额累计不超过订单明细已购数量和实付金额。
- 售后审核时扣除其他已批准且未结束申请占用的数量和金额。
...
- [`sql/schema.sql`](../../sql/schema.sql) 只用于空库初始化。
- 脚本中的 RBAC 种子数据按空库执行设计，不是可重复运行的增量迁移。
- 生产或已存在数据的环境必须使用版本化数据库迁移脚本，不得重复执行 `schema.sql`。
