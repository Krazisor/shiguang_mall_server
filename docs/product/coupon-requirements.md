...
券模板状态使用 `DRAFT`、`ACTIVE`、`PAUSED`、`ENDED`、`ARCHIVED`：
...
`ARCHIVED` 用于隐藏废弃模板，不是删除。只允许草稿或已结束模板归档；运行中模板必须先结束。归档后不允许恢复或修改，只能查看历史并复制为新草稿。默认模板列表隐藏归档记录，显式选择“已归档”仍可查询。归档不撤销已领券，不删除范围、兑换码、核销和预算记录，也不改变结算或退款返券。
...
| 页面 | 必须展示 | 主要操作 |
| --- | --- | --- |
| 券模板列表 | 类型、利益、范围、有效期、发行/领取/核销、状态、预算 | 创建、复制、暂停、结束、归档、筛选已归档 |
...
券模板列表默认隐藏 `ARCHIVED`，并提供“已归档”状态筛选；`DRAFT/ENDED` 可填写原因后归档。归档入口不得使用“删除”文案或删除图标。

平台端除平台券和全局活动管理外，还需提供店铺活动治理、跨店核销检索、资金承担审计、异常预占和预算对账。只读运营人员不能发布、归档、撤销或定向发券。
...
22. 草稿或结束模板归档后从默认列表隐藏但可显式查询，已领取券继续可用，预算与历史对账不变。

## 22. 完成定义

- 产品、数据库、API、权限和运营文档使用相同枚举、金额公式、状态机和返券政策。
- 增量迁移不回写任何已发布脚本；模板归档通过 `scheme9.sql` 扩展状态检查约束。
