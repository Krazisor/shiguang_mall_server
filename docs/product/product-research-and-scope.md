...
本文在现有数据库、Java 实体和技术栈基础上，确定时光商城两个开发阶段共同遵循的产品范围。它回答四个问题：系统服务谁、解决什么流程、当前必须实现什么、哪些常见商城能力暂不实现。
...
| 模块 | 当前模型 | 必须提供的产品能力 | 阶段 |
| --- | --- | --- | --- |
| 身份认证 | `sys_user` | 注册、登录、退出、会话、资料、账号状态检查 | 一期 |
| 地址 | `user_address` | 增删改查、唯一默认地址 | 一期 |
| 平台 RBAC | `sys_role` 等 4 表 | 角色、权限、平台角色分配、用户停用/解锁 | 二期 |
| 店铺 | `shop` | 平台创建和维护店铺、状态治理、公开信息 | 一期 |
| 店铺成员 | `shop_user` | 添加成员、改角色、启停成员 | 二期 |
| 类目属性品牌 | `product_category` 等 3 表 | 类目树、叶子属性模板、品牌维护 | 一期 |
| 商品 | `product_spu` 等 4 表 | SPU/SKU、属性、审核、上下架、历史、禁售 | 一期基础，二期治理完善 |
| 库存 | `inventory_stock`、`inventory_transaction` | 查询、入库、下单锁定、取消释放、发货扣减、调整、流水 | 一期闭环，二期审计 |
| 购物车 | `cart_item` | 加购、改数量、选择、删除、按店铺展示、失效提示 | 一期 |
| 交易订单 | `trade_order` 等 4 表 | 结算预览、跨店提交、取消、统一支付、商家发货、确认收货、历史 | 一期 |
| 钱包支付 | `wallet_account` 等 3 表 | 钱包查询、模拟充值、支付尝试、余额扣减、流水 | 一期 |
| 售后退款 | `after_sale_request` | 申请、撤销、审核、退货信息、收货、退款、失败重试 | 二期 |
| 自动任务 | 订单/支付/退款现有字段 | 超时取消、自动完成、退款重试、聚合对账 | 二期 |
...
| 运费 | 当前无运费模板，两个阶段统一为 `0.00`；字段保留但前端不得提交运费 |
