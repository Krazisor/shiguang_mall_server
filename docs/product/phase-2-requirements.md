...
二期在一期正向交易闭环上，补齐当前 26 表数据模型已经支持但一期未完整交付的权限治理、运营审计、售后退款和自动任务。完成后，系统能够演示从平台治理、店铺分工、正常交易到逆向退款的完整生命周期，并具备必要的幂等、对账和故障恢复能力。

二期不是营销扩展期，不增加优惠、评价、真实支付、结算、推荐等新领域。
...
一期种子角色继续保留。二期可以创建细分角色，但角色权限模型仍是静态权限代码，不做字段级、行表达式或临时授权。
...
| 退货物流 | 已批准的退货退款填写承运商和运单号 | 只允许 `WAITING_RETURN`，一次提交 |
...
申请数量和申请金额上限遵循数据库文档的累计退款公式。已有 `PENDING`、`WAITING_RETURN`、`REFUNDING` 申请会占用额度；`REJECTED` 和 `CANCELLED` 不占用；`COMPLETED` 已计入累计退款。

### 2.6 平台运营只读查询

平台为排障提供跨店订单、支付、钱包流水和售后只读查询，但不新增跨店代操作。平台人员不能通过只读权限替商家发货、批准售后或直接修改买家钱包。
...
活跃售后指 `PENDING`、`WAITING_RETURN`、`REFUNDING`。待发货子订单存在活跃售后时禁止商家发货；待收货子订单存在活跃售后时禁止买家手工确认和系统自动完成。申请被拒绝、撤销或完成后，正向履约动作重新按当前订单状态开放。
...
7. 相同售后幂等键并发提交三次只产生一条申请；相同退款重试只产生一条钱包流水。
8. 退款执行被模拟为失败后状态为 `REFUNDING/FAILED`，手动重试成功并沿用原 `refundNo`。
9. 超时未支付交易被任务取消并释放库存；任务重复执行不产生额外流水。
10. 人工篡改测试库聚合余额后，对账任务能报告钱包差异但不会自动修改余额。

## 7. 二期完成定义

- [二期接口文档](../api/phase-2-api.md) 所有接口实现并通过权限、契约和集成测试。
- 售后状态机、退款金额上限、库存变化和钱包流水覆盖单元与并发测试。
- 自动任务在单实例和双实例测试中均不重复处理。
- 一期回归测试全部通过，二期没有改变一期接口的已有语义。
